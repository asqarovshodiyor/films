...
films.map(x => {
    ...
    elId.setAttribute('class', 'ddd')
    let elImg = document.createElement('img')
    let elDate = document.createElement('p')
    elDate.setAttribute('class', 'date')
    let elGens = document.createElement('h4')
    let blockQuote = document.createElement('blockquote')
    elImg.setAttribute('width', '240px')


    elName.textContent = x.title;
    let data = new Date(x.release_date)
    let year = data.getFullYear();
    let month = String(data.getMonth()).padStart(2, "0");
    let day = data.getDate();   
    elDate.textContent = (`${year}.${month}.${day}`)
    elGens.textContent = x.genres
    elId.textContent ='id: ' + x.id
    elImg.setAttribute('src', x.poster)
    blockQuote.textContent = x.overview

    elLI.appendChild(elImg)
    elLI.appendChild(elName)
    elLI.appendChild(elGens)
    elLI.appendChild(elDate)
    elLI.appendChild(elId)
    unList.appendChild(elLI)
    elLI .appendChild(blockQuote)
    unList.appendChild(elLI)



    // let elForm = document.querySelector('#form')
    // let elOPt = elForm.querySelector('#option')

    // elForm.addEventListener('submit', function (evt) {
    //     evt.preventDefault()
    //     for (let i = 0; i < films.length; i++) {
    //         if (films[i].genres.includes( ) == elOPt.value) {
